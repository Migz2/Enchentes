...
def remover_coluna(coluna_para_remover):
    ...
    df = pd.read_csv("nivel_extraido.csv")
    ...
    if coluna_para_remover in df.columns:
        ...
        df.to_csv("nivel_extraido.csv", index=False)  # Salva o CSV sem a coluna removida
...
csv_destino = os.path.join(base_dir, "dados_api.csv")  # Caminho correto


# Executa a função
coluna_data = "Data"
csv_destino = "nivel_extraido.csv"
ordenar_csv_por_data(csv_destino, coluna_data)
coluna_para_remover = "Data"
remover_coluna(coluna_para_remover)
origem_csv = "nivel_extraido.csv"
destino_csv = "dados_api.csv"
